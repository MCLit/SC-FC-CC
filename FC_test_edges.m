...
load('F_ID.mat', 'F_EF', 'group_f') % variables: SWR model, SC data prior to standardisation and PCA
cog = 1;% names of the output files
fname_mat = sprintf('testF%d_b.mat', cog); % output of permutations
fname_csv1 =  sprintf('F%d.csv', cog); % output of masking

fname_csvp =  sprintf('F%dp.csv', cog); % output in 7-network space
fname_csvn =  sprintf('F%dn.csv', cog);

mdl = F_EF;
Y = table2array(mdl.Variables(:,end)); % response variable

% standardise scores
[Z,MU,SIGMA] = zscore(group_f); 
...
disp('competed masking connections')