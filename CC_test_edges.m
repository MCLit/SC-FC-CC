...
load('J_ID.mat', 'J_SEQ', 'group_f', 'group_s') % variables: SWR model, SC data prior to standardisation and PCA
cog = 1;% names of the output files
fname_mat = sprintf('testJ%d_b.mat', cog); % output of permutations
fname_csv1 =  sprintf('JS%d.csv', cog); % output of masking
fname_csv2 =  sprintf('JF%d.csv', cog); % output of masking

fname_csvp =  sprintf('JS%dp.csv', cog); % output in 7-network space
fname_csvn =  sprintf('JS%dn.csv', cog);
Ffname_csvp =  sprintf('JF%dp.csv', cog); % output in 7-network space
Ffname_csvn =  sprintf('JF%dn.csv', cog);
...
disp('competed: generating null distribution of weights')
...
disp('competed computing critical scores')

%% masking and scaling of connections
...
S = Betas_Hauffe(1:38503);
F = Betas_Hauffe(38504:end);

n = 278; M = zeros(n,n); l = tril(M,-1); ind = find(tril(ones(n,n),-1));
M(ind) = S'; K = M';

SC_th_mdl_brainspace = M+K; % full matrix

n = 278; M = zeros(n,n); l = tril(M,-1); ind = find(tril(ones(n,n),-1));
M(ind) = F'; K = M';

FC_th_mdl_brainspace = M+K; % full matrix


csvwrite(fname_csv1, SC_th_mdl_brainspace) % save result as csv file
csvwrite(fname_csv2, FC_th_mdl_brainspace) % save result as csv file
